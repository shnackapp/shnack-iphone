...
@interface POPDTableViewController () <UIGestureRecognizerDelegate>
...
@implementation POPDTableViewController
@synthesize delegate;
...
- (void)viewDidLoad
{
    ...
    self.headerCell = [self.tableView dequeueReusableCellWithIdentifier:cellIdentifier];
...
- (void)updateHeaderView {
    if (!self.startingScrollIsSet) {
        self.startingScrollY = self.tableView.contentOffset.y;
        self.startingScrollIsSet = YES;
    }
    
    NSIndexPath *indexPath1 = [self.tableView indexPathForRowAtPoint:CGPointMake(0, self.tableView.contentOffset.y - self.startingScrollY)];
    ...
    NSIndexPath *indexPath2 = [self.tableView indexPathForRowAtPoint:CGPointMake(0, self.tableView.contentOffset.y - self.startingScrollY + self.headerCell.frame.size.height)];
    ...
    if(indexPath2.row > 0) {
        ...
        if (self.currentHeaderSection != indexPath2.section) {
            ...
            if ([self.delegate respondsToSelector:@selector(willDisplayOpenedCategoryCell:atIndexPath:)]) {
                [self.delegate willDisplayOpenedCategoryCell:self.headerCell atIndexPath:[NSIndexPath indexPathForRow:0 inSection:self.currentHeaderSection]];
            } else {
                self.headerCell.labelText.text = [self.sectionsArray[self.currentHeaderSection] objectAtIndex:0];
            }
        }
        self.headerView.hidden = NO;
        CGRect frame = CGRectMake(0, self.tableView.contentOffset.y - self.startingScrollY, [[UIScreen mainScreen] bounds].size.width, self.headerView.frame.size.height);
        ...
        if (indexPath2.row == 0 && indexPath2.section == 0) {
            frame = CGRectMake(0, self.tableView.contentOffset.y - self.startingScrollY, [[UIScreen mainScreen] bounds].size.width, self.headerView.frame.size.height);
        } else {
            CGRect nextRect = [self.tableView rectForRowAtIndexPath:indexPath2];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (self.currentHeaderSection == section) {
        if ([self.delegate respondsToSelector:@selector(willDisplayOpenedCategoryCell:atIndexPath:)]) {
            [self.delegate willDisplayOpenedCategoryCell:self.headerCell atIndexPath:[NSIndexPath indexPathForRow:0 inSection:section]];
        } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isLoading) {
        ...
        if ([self sectionIsLeaf:indexPath.section]) {
            ...
            if ([self.delegate respondsToSelector:@selector(willDisplayLeafCell:atIndexPath:)]) {
                [self.delegate willDisplayLeafCell:cell atIndexPath:indexPath];
            } else {
            ...
            if ([self.delegate respondsToSelector:@selector(willDisplayOpenedCategoryCell:atIndexPath:)]) {
                [self.delegate willDisplayOpenedCategoryCell:cell atIndexPath:indexPath];
            } else {
                cell.labelText.text = [[self.sectionsArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
            }
        } else {
            ...
            if ([self.delegate respondsToSelector:@selector(willDisplayClosedCategoryCell:atIndexPath:)]) {
                [self.delegate willDisplayClosedCategoryCell:cell atIndexPath:indexPath];
            } else {
                cell.labelText.text = [[self.sectionsArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
            }
        }
        ...
        if ([self.delegate respondsToSelector:@selector(willDisplayLeafSubCell:atIndexPath:)]) {
            [self.delegate willDisplayLeafSubCell:cell atIndexPath:indexPath];
        } else {
            cell.labelText.text = [[self.sectionsArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
        }
    }
...
- (void)didSelectHeaderView:(id)obj {
    self.headerView.hidden =  YES;
    [self tableView:self.tableView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:self.currentHeaderSection]];
}
...
- (void)tableView:(UITableView *)tableView didEndDisplayingCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.sectionIsOpening) {
        [[self tableView] scrollToRowAtIndexPath:self.openingIndexPath atScrollPosition:UITableViewScrollPositionTop animated:YES];
        self.sectionIsOpening = NO;
    }
}
